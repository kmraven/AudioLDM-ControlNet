...
from trainer.trainer_lsc import Trainer
from modules.optimization import AdamW, get_cosine_schedule_with_warmup
import pdb
from modules.lsc import ConceptBranch, ConditionalSimNet
# beat dimension
...
L = 10
num_concepts = 8
beta = 0.0 # without beatloss
exp_name = f"seglen{seglen}_L{L}_batchsize{batch_size}_concepts{num_concepts}_beta{beta}"
...
def main():
    ...
    config = CusConfig("config/pdl_seg10_L10_LSC.json", exp_name)
    config.videos_dir = data_dir
    config.batch_size = batch_size
    config.num_frames = L
    config.output_dir = output_dir
    config.parse_args()
    #config.tb_log_dir = os.path.join('logs')
    ...
    concept_branch = ConceptBranch(
        ...
        # embedding_dim=config.embed_dim * config.num_frames * 2
        embedding_dim=config.embed_dim * 2 # middle FC layer's dimension
    )
    conditional_sim = ConditionalSimNet(
        n_conditions=num_concepts,
        # embedding_size=config.embed_dim * config.num_frames,
        embedding_size=config.embed_dim,
        learnedmask=True, #learnable mask
        prein=False #all masks randomly initialized to be 10% masked
    )
    ...
    optimizer_grouped_params = [
        ...
        {'params': concept_branch.parameters(), 'lr': config.noclip_lr},
        {'params': conditional_sim.parameters(), 'lr': config.noclip_lr},
    ]
    ...
    trainer = Trainer(model, loss, metrics, optimizer,
                      ...
                      conditional_sim=conditional_sim)
    ...
    if config.prune_after_training:
        print("Pruning unused concepts...")
        trainer._prune_concepts(epoch=config.num_epochs)
