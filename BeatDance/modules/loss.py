...
class CLIPLoss(nn.Module):
    ...
    def forward(self, sims, logit_scale):
        ...
        logits = sims * logit_scale # scale similarity matrix by some exponential function
        
        t2v_log_sm = F.log_softmax(logits, dim=1)
        t2v_neg_ce = torch.diag(t2v_log_sm) #corresponding framewise similarity between two modalities
        #  frames in the entire batch is all aggregated (despite coming from different samples)
        t2v_loss = -t2v_neg_ce.mean() # you want to maximize similarity, therefore you minimize the negative likelihood of it,
