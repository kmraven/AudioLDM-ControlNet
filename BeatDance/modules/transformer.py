...
class PoseTransformer(nn.Module):
    def __init__(self, config: Config):
        ...
        video_pose_encoder_layer = nn.TransformerEncoderLayer(
            d_model=config.embed_dim, nhead=config.num_mha_heads
        )
        self.device = self._prepare_device()
        self.total_len = torch.tensor(630).to(self.device)
        self.video_pose_encoder_first_layers = []
        for i in range(config.num_frames):
            self.video_pose_encoder_first_layers.append(
                nn.TransformerEncoder(video_pose_encoder_layer, num_layers=1).to(
                    self.device
                )
            )
        self.video_pose_encoder_second_layer = nn.TransformerEncoder(
            video_pose_encoder_layer, num_layers=1
        ).to(self.device)
    ...
    def forward(self, pose_data):
        ...
        pose_data_second = torch.zeros((pose_data.shape[0], 30, pose_data.shape[2])).to(
            self.device
        )
        for i in range(self.num_frames):
            result = self.video_pose_encoder_first_layers[i](
                pose_data[:, interval[i] : interval[i + 1], :]
            )
