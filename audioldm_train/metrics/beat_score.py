...
- tempo_difference is implemented here because it is not provided by Textual Inversion
"""
...
def beat_scores(gt, syn):
    ...
    """
    common_length = min(len(gt), len(syn))
    gt = gt[:common_length]
    syn = syn[:common_length]
    total_beats = sum(gt)
    cover_beats = sum(syn)
    hit_beats = 0
...
def motion_peak_onehot(joints):
    ...
    if joints.ndim != 3:
        raise ValueError(
            f"Expected motion with shape [frames, joints, coordinates], got {joints.shape}"
        )
...
def alignment_score(music_beats, motion_beats, sigma=3):
    ...
    """
    common_length = min(len(music_beats), len(motion_beats))
    music_beats = np.asarray(music_beats[:common_length])
    motion_beats = np.asarray(motion_beats[:common_length])

    if music_beats.sum() == 0 or motion_beats.sum() == 0:
        return 0.0
...
def calculate_audio_beat_score(gt_audio, gen_audio, sr):
    gt_audio = _to_numpy(gt_audio)
...
class BeatScoreCalculator:
    """Backward-compatible wrapper for audio-only beat metrics."""

    def calculate(self, gt_audio, gen_audio, sr):
        return calculate_audio_beat_score(gt_audio, gen_audio, sr)
